...
const { validatePersonIdsQuery, validatePlayerIdParam } = require('./peopleQueryValidation.cjs');
...
describe('validatePersonIdsQuery', () => {
  ...
  it('accepts exactly 50 ids', () => {
    ...
    expect(validatePersonIdsQuery(ids)).toEqual({ ok: true, trimmedIds: ids });
  });

  it('rejects 51 ids', () => {
    const ids = Array.from({ length: 51 }, (_, i) => i + 1).join(',');
    const r = validatePersonIdsQuery(ids);
    expect(r.ok).toBe(false);
...
describe('validatePlayerIdParam', () => {
  it('accepts digits only', () => {
    expect(validatePlayerIdParam('660271')).toEqual({ ok: true, playerId: '660271' });
  });
