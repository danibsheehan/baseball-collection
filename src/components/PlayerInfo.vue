...
const uniformDisplay = computed(() => {
  const roster =
    props.jerseyNumber != null && String(props.jerseyNumber).trim() !== ''
      ? String(props.jerseyNumber).trim()
      : '';
  const primary =
    props.playerInfo?.primaryNumber != null && String(props.playerInfo.primaryNumber).trim() !== ''
...
const positionLabel = computed(() => props.playerInfo?.primaryPosition?.name || '—');

const batLabel = computed(() => props.playerInfo?.batSide?.description || '—');

const throwLabel = computed(() => props.playerInfo?.pitchHand?.description || '—');
