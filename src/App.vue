            ...
            <nav class="teams__nav" aria-label="Major League Baseball teams">
              <div class="teams__picker-print-frame">
                <div class="teams__picker-panel">
                  ...
                  <template v-else>
                    <div class="teams__sections" :class="teamsSectionsLayoutClass">
                      <section
                        v-for="section in displayTeamSections"
                        :key="section.id"
                        class="teams__section"
                      >
                        <h3 v-if="section.label" class="teams__section-title" :id="section.id">
                          {{ section.label }}
                        </h3>
                        <div
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div class="album__roster-filter" role="group" aria-label="Roster view">
                      <button
                        type="button"
                  ...
                  <p v-else-if="!players.length" class="album__results-empty">
...
body {
  ...
  background-image:
    radial-gradient(rgba(72, 58, 42, 0.03) 0.55px, transparent 0.65px), var(--app-body-fiber-a),
...
@media (prefers-color-scheme: dark) {
  body {
    ...
    background-image:
      ...
      var(--app-body-fiber-a), var(--app-body-fiber-b),
      repeating-linear-gradient(
        180deg,
        transparent 0,
        transparent 6px,
...
.app__rail--paper::before {
  background-image: radial-gradient(rgba(72, 58, 42, 0.052) 0.68px, transparent 0.84px);
...
@media (prefers-color-scheme: dark) {
  .app__rail--paper::before {
    background-image: radial-gradient(rgba(255, 252, 246, 0.055) 0.58px, transparent 0.78px);
...
.album__results-empty-action {
  background: color-mix(
    in srgb,
    var(--color-surface-elevated, #f7f3ea) 88%,
    var(--color-ui-crimson)
  );
...
.teams__picker-print-frame::before {
  background:
    ...
    linear-gradient(currentColor, currentColor) calc(100% - 2px) calc(100% - 2px) / 10px 1px
      no-repeat,
    linear-gradient(currentColor, currentColor) calc(100% - 3px) calc(100% - 3px) / 1px 10px
      no-repeat;
  content: '';
  inset: 0;
  opacity: 0.48;
...
.teams__picker-panel {
  ...
  max-width: 100%;
  padding: calc(var(--teams-pack-strip-h) + clamp(var(--space-4), 2.5vw, var(--space-6)))
...
@keyframes album-card-reveal {
  0% {
    ...
    transform: translate3d(var(--deal-from-x, 0px), var(--deal-from-y, 0px), 0)
      ...
      scale3d(calc(0.84 * var(--deal-lead-boost, 1)), calc(0.84 * var(--deal-lead-boost, 1)), 1);
  }
  ...
  38% {
    transform: translate3d(
        calc(var(--deal-from-x, 0px) * 0.08),
        calc(var(--deal-from-y, 0px) * 0.08),
        0
      )
  ...
  100% {
    ...
    transform: translate3d(0, 0, 0) rotateY(0deg)
      rotateZ(calc(var(--deal-twist, 0deg) * var(--deal-rest, 0.24))) scale3d(1, 1, 1);
  }
}
...
.album__card-deal--animate[data-deal-act='both'] {
  animation:
    album-card-reveal var(--deal-anim-duration, 1960ms) ease-out var(--deal-delay, 0ms) forwards,
    album-deal-act3-cap-settle 0.52s cubic-bezier(0.34, 1, 0.45, 1) var(--deal-cap-delay, 0ms)
      forwards;
}
...
.album__card-deal--many.album__card-deal--animate[data-deal-act='both'] {
  animation:
    album-card-reveal var(--deal-anim-duration, 1380ms) ease-out var(--deal-delay, 0ms) forwards,
    album-deal-act3-cap-settle 0.48s cubic-bezier(0.34, 1, 0.45, 1) var(--deal-cap-delay, 0ms)
      forwards;
}
...
.album__results--title::before {
  background:
    ...
    linear-gradient(175deg, var(--album-roster-pennant-a) 0%, var(--album-roster-pennant-b) 100%);
  box-shadow:
...
@media (prefers-color-scheme: dark) {
  ...
  @supports (color: color-mix(in srgb, red 10%, blue 90%)) {
    .album__results--title {
      color: color-mix(in srgb, var(--theme-heading, var(--color-text)) 45%, var(--color-text) 55%);
    }
  }
}
